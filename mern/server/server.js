...
app.use("/record", records); // TODO: This will have to be changed, along with client components, as it's using the basic framework from MongoDB's guide


import db from "./db/connection.js";
app.get("/__debug", async (_req, res) => {
  const names = await db.listCollections().toArray(); // TODO: we are going to have to add actors database, and the other ones when we are ready
